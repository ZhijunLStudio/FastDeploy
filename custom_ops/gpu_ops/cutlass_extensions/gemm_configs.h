...
enum class CutlassTileConfigSM100 {
  ...
  // CtaShape128x64x128B and CtaShape256x64x128B for specific FP4 grouped gemm
  // cases. These are already covered by the list above if general suffices. If
  // they need distinct enum values, they should be added. For now, keeping the
  // enum concise with unique shapes mentioned for general use.
};
...
struct CutlassGemmConfig {
  ...
  // These might need to become SM100-specific if Blackwell introduces new
  // concepts.
  CutlassTileConfigSM100 tile_config_sm100 =
      CutlassTileConfigSM100::ChooseWithHeuristic;
  // MainloopScheduleType mainloop_schedule_sm100 = MainloopScheduleType::AUTO;
  // // Example if SM100 has different types EpilogueScheduleType
  // epilogue_schedule_sm100 = EpilogueScheduleType::AUTO; // Example
  // ClusterShape cluster_shape_sm100 = ClusterShape::ClusterShape_1x1x1; //
  // Example
  bool is_sm100 = false;

  CutlassGemmConfig() : is_sm90(false), is_sm100(false) {}

  CutlassGemmConfig(CutlassTileConfig tile_config,
  ...
  CutlassGemmConfig(CutlassTileConfigSM90 tile_config_sm90_in,
                    MainloopScheduleType mainloop_schedule_in,
                    EpilogueScheduleType epilogue_schedule_in,
                    ClusterShape cluster_shape_in)
  ...
  // Using existing MainloopScheduleType, EpilogueScheduleType, ClusterShape for
  // now. These might need to be new SM100-specific types if Blackwell's TMA
  // differs significantly.
  CutlassGemmConfig(CutlassTileConfigSM100 tile_config_sm100_in,
                    MainloopScheduleType mainloop_schedule_in,
                    EpilogueScheduleType epilogue_schedule_in,
                    ClusterShape cluster_shape_in)
      : tile_config_sm100(tile_config_sm100_in),
        mainloop_schedule(
            mainloop_schedule_in)  // Potentially use mainloop_schedule_sm100 if
                                   // types diverge
        ,
        epilogue_schedule(
            epilogue_schedule_in)  // Potentially use epilogue_schedule_sm100
        ,
  ...
  std::string toString() const {
    ...
    if (is_sm100 &&
        tile_config_sm100 !=
            cutlass_extensions::CutlassTileConfigSM100::ChooseWithHeuristic) {
    ...
    } else if (is_sm90 && tile_config_sm90 !=
                              cutlass_extensions::CutlassTileConfigSM90::
                                  ChooseWithHeuristic) {
      ...
      tactic << "\n\tstyle=TMA_SM90"
             ...
             << "\n\tepi sched: " << (int)epilogue_schedule;
               ...
               cutlass_extensions::CutlassTileConfig::ChooseWithHeuristic) {
      assert(!is_sm90 && !is_sm100 &&
             "Invalid cutlass GEMM config: Compatible");
      tactic << "\n\tstyle=compatible"
  ...
  void fromString(const std::string& str) {
    ...
    while (std::getline(stream, line)) {
      if (line.find("style=TMA_SM100") != std::string::npos) {
        ...
        tile_config_sm100 =
            static_cast<cutlass_extensions::CutlassTileConfigSM100>(
                std::stoi(line.substr(line.find(':') + 1)));
        std::getline(stream, line);
        cluster_shape = static_cast<cutlass_extensions::ClusterShape>(
            std::stoi(line.substr(line.find(':') + 1)));
        std::getline(stream, line);
        mainloop_schedule =
            static_cast<cutlass_extensions::MainloopScheduleType>(
                std::stoi(line.substr(line.find(':') + 1)));
        std::getline(stream, line);
        epilogue_schedule =
            static_cast<cutlass_extensions::EpilogueScheduleType>(
                std::stoi(line.substr(line.find(':') + 1)));
      } else if (line.find("style=TMA_SM90") !=
                 std::string::npos) {  // Check for SM90 specific first
        is_sm90 = true;
        is_sm100 = false;
        std::getline(stream, line);
        tile_config_sm90 =
            static_cast<cutlass_extensions::CutlassTileConfigSM90>(
                std::stoi(line.substr(line.find(':') + 1)));
        std::getline(stream, line);
        cluster_shape = static_cast<cutlass_extensions::ClusterShape>(
            std::stoi(line.substr(line.find(':') + 1)));
        ...
        is_sm100 = false;
        std::getline(stream, line);
        tile_config = static_cast<cutlass_extensions::CutlassTileConfig>(
            std::stoi(line.substr(line.find(':') + 1)));
        std::getline(stream, line);
        stages = std::stoi(line.substr(line.find(':') + 1));
        std::getline(stream, line);
        split_k_style = static_cast<cutlass_extensions::SplitKStyle>(
            std::stoi(line.substr(line.find(':') + 1)));
        std::getline(stream, line);
