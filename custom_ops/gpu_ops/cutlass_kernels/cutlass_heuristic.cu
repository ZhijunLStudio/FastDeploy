...
TileShape get_cta_shape_for_config(CutlassTileConfig tile_config) {
  switch (tile_config) {
    ...
    case CutlassTileConfig::CtaShape64x64x64_WarpShape32x32x64:
    ...
    case CutlassTileConfig::CtaShape16x256x128_WarpShape16x64x128:
      return TileShape{16, 256};
    default:
      throw("[get_grid_shape_for_config] Invalid config");
  }
}

bool is_valid_split_k_factor(int64_t const m,
                             int64_t const n,
                             int64_t const k,
...
// 128. This is purely to improve compilation speed.
...
std::vector<CutlassGemmConfig> get_candidate_configs(
  ...
  if (sm == 90 && (config_type_param & CutlassGemmConfig::HOPPER)) {
    std::vector<CutlassTileConfigSM90> tiles =
        get_candidate_tiles_sm90(sm, config_type_param);
    ...
    return candidate_configs;
             ...
             (config_type_param &
              CutlassGemmConfig::BLACKWELL))  // Assuming SM100 for Blackwell
  {
    std::vector<CutlassTileConfigSM100> tiles =
        get_candidate_tiles_sm100(sm, config_type_param);
    std::vector<CutlassGemmConfig> candidate_configs;

    for (auto const& tile_config_sm100 : tiles) {
      // SM100 uses MainloopScheduleType::AUTO, EpilogueScheduleType::AUTO
      // similar to SM90. Cluster shapes are also handled similarly.
      CutlassGemmConfig config(tile_config_sm100,
                               MainloopScheduleType::AUTO,
                               EpilogueScheduleType::AUTO,
                               ClusterShape::ClusterShape_1x1x1);
      candidate_configs.push_back(config);
      ...
      if (has_m_mcast) {
        CutlassGemmConfig mcast_m_config(tile_config_sm100,
                                         MainloopScheduleType::AUTO,
                                         EpilogueScheduleType::AUTO,
                                         ClusterShape::ClusterShape_2x1x1);
      ...
      if (has_n_mcast) {
        CutlassGemmConfig mcast_n_config(tile_config_sm100,
                                         MainloopScheduleType::AUTO,
                                         EpilogueScheduleType::AUTO,
                                         ClusterShape::ClusterShape_1x2x1);
      ...
      if (has_m_mcast && has_n_mcast) {
        CutlassGemmConfig mcast_mn_config(tile_config_sm100,
                                          MainloopScheduleType::AUTO,
                                          EpilogueScheduleType::AUTO,
                                          ClusterShape::ClusterShape_2x2x1);
  ...
  std::vector<CutlassTileConfig> tiles =
      get_candidate_tiles(sm, config_type_param);
  std::vector<CutlassGemmConfig>
      candidate_configs;  // Already declared above for SM90 path, ensure scope
                          // is correct or redeclare if necessary.
                          //  It's fine here as it's within an else if / else
                          //  block.
