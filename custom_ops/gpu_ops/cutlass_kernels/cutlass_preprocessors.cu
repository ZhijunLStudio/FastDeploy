...
struct getLayoutDetails<
    cutlass::layout::ColumnMajorTileInterleave<RowsPerTile,
                                               ColumnsInterleaved>> {
  LayoutDetails operator()() {
    LayoutDetails layout_details;
    layout_details.layoutB = LayoutDetails::Layout::COLUMN_MAJOR;
...
LayoutDetails getLayoutDetailsForArchAndQuantType() {
  using CompileTraits =
      cutlass::gemm::kernel::LayoutDetailsB<TypeA, TypeB, cutlassArch>;
...
LayoutDetails getLayoutDetailsForArch(QuantType quant_type) {
  ...
  switch (quant_type) {
    case QuantType::W8_A16:
      details = getLayoutDetailsForArchAndQuantType<cutlassArch,
                                                    cutlass::half_t,
                                                    uint8_t>();
      break;
    ...
    case QuantType::W4_AFP8:
      ...
      break;
    default:
      PADDLE_THROW("Unsupported quantization type");
  }
  return details;
}
...
// after we execute LDSM. It counteracts the effect of the data being of
// different widths. For more information about the expected layouts, see the
// MMA section in the PTX docs.
...
void permute_B_rows_for_mixed_gemm(int8_t* permuted_quantized_tensor,
  ...
  PADDLE_ENFORCE(arch_version >= 75,
...
void subbyte_transpose_impl(int8_t* transposed_quantized_tensor,
  ...
  uint8_t const* input_byte_ptr =
      reinterpret_cast<uint8_t const*>(quantized_tensor);
  ...
  for (size_t expert = 0; expert < num_experts; ++expert) {
    ...
    for (size_t row_tile_start = 0; row_tile_start < num_rows;
      ...
      for (size_t col_tile_start_byte = 0; col_tile_start_byte < col_bytes;
        ...
        int const row_limit_trans =
            std::min(row_tile_start_trans + M_TILE_L1, num_cols);
        int const col_limit_trans =
...
void subbyte_transpose(int8_t* transposed_quantized_tensor,
                       ...
                       QuantType quant_type) {
  if (quant_type == QuantType::W8_A16) {
    subbyte_transpose_impl<QuantType::W8_A16>(
        transposed_quantized_tensor, quantized_tensor, shape);
  } else if (quant_type == QuantType::W4_A16) {
    subbyte_transpose_impl<QuantType::W4_A16>(
        transposed_quantized_tensor, quantized_tensor, shape);
  } else if (quant_type == QuantType::W4_AFP8) {
    subbyte_transpose_impl<QuantType::W4_AFP8>(
        transposed_quantized_tensor, quantized_tensor, shape);
  } else {
...
void add_bias_and_interleave_int8s_inplace(int8_t* int8_tensor,
  ...
  PADDLE_ENFORCE(num_elts % 4 == 0,
                 "Dimensions of int8 tensor must be a multiple of 4 for "
                 "register relayout");
...
void add_bias_and_interleave_int4s_inplace(int8_t* packed_int4_tensor,
  ...
  for (size_t ii = 0; ii < num_bytes; ++ii) {
    ...
    PADDLE_ENFORCE(transformed_first_elt >= 0 && transformed_first_elt <= 15,
  ...
  PADDLE_ENFORCE(num_bytes % 4 == 0,
                 "Dimensions of int4 tensor must be a multiple of 8 for "
                 "register relayout");
...
void preprocess_weights_for_mixed_gemm(int8_t* preprocessed_quantized_weight,
  ...
  std::copy(row_major_quantized_weight,
            row_major_quantized_weight + num_bytes,
            src_buf.begin());

  // Works on row major data, so issue this permutation first.
  if (details.uses_imma_ldsm) {
    permute_B_rows_for_mixed_gemm(
        dst_buf.data(), src_buf.data(), shape, quant_type, arch);
  ...
  if (details.columns_interleaved > 1) {
    interleave_column_major_tensor(
        dst_buf.data(), src_buf.data(), shape, quant_type, details);
    src_buf.swap(dst_buf);
  }

  if (arch >= 70 && arch < 90) {
    add_bias_and_interleave_quantized_tensor_inplace(
        src_buf.data(), num_elts, quant_type);
  }
...
full int range and assumes the zero-point is zero and will automatically
construct the scales.

    It always quantizes the last axis of the tensor. For 3-D tensors, it
operates in "batched" mode where the tensor is viewed as a stack of matrices and
a scale is produced for each column of every matrix.

Outputs
    processed_quantized_weight - quantized AND processed weight for GEMM. This
MUST be used with the CUTLASS GEMM unprocessed_quantized_weight - quantized but
unprocessed weights. Useful for reference checking. scale_ptr - scales for the
quantized weight.

    Note that the returned quantized_weights will be preprocessed in a way to
accelerate the mixed type GEMM. The data layout may not make sense if printed.

    Shapes:
      quant_type == int8:
        If weight is a [m,n] matrix, quantized_weights will have shape [m,n] and
scales of shape [n] If weight is a [b,m,n] tensor, unprocessed_quantized_weight
will have shape [b,m,n] and scales of shape [b,n] quant_type == int4: If weight
is a [m,n] matrix, quantized_weights will have shape [m, ceil(n/2)] and scales
of shape [n] If weight is a [b,m,n] tensor, unprocessed_quantized_weight will
have shape [b,m, ceil(n/2)] and scales of shape [b,n]

      The quantized_weight will be of type torch.int8 and have two int4 values
packed in a single byte. This is the reason for halving the shape. At the time
of writing this code, there was not an elegant way to handle this kind of
batched quantization using torch's quantized tensors (to the best of the
author's knowledge). Scale tensors must have a dimension of 1, which breaks the
semantics we need for batched weights.
  */
...
void symmetric_quantize(int8_t* processed_quantized_weight,
                        ...
                        QuantType quant_type,
                        bool force_interleave) {
  PADDLE_ENFORCE(processed_quantized_weight,
                 "Processed quantized tensor is NULL");
  ...
  for (int expert = 0; expert < num_experts; ++expert) {
    WeightType const* current_weight =
        input_weight_ptr + expert * input_mat_size;
    ...
    for (int ii = 0; ii < num_rows; ++ii) {
      ...
      for (int jj = 0; jj < num_cols; ++jj) {
        per_col_max[jj] =
            std::max(per_col_max[jj], std::abs(float(current_weight_row[jj])));
      }
    }
    ...
    for (int ii = 0; ii < num_rows; ++ii) {
      ...
      for (int jj = 0; jj < bytes_per_out_col; ++jj) {
        if (bits_per_weigtht_element == 8) {
          ...
          float const scaled_weight =
              (col_scale != 0.0f) ? round(weight_elt / col_scale) : 0.0f;
          const int8_t clipped_weight =
              int8_t(std::max(-128.f, std::min(127.f, scaled_weight)));
          ...
          for (int packed_idx = 0; packed_idx < 2; ++packed_idx) {
            ...
            if (input_idx < num_cols) {
              ...
              const int8_t clipped_weight =
                  std::max(-8, std::min(7, int_weight));
  ...
  preprocess_weights_for_mixed_gemm(processed_quantized_weight,
                                    unprocessed_quantized_weight,
                                    shape,
                                    quant_type,
                                    force_interleave);
}

template void symmetric_quantize<half, float>(int8_t*,
                                              int8_t*,
                                              half*,
                                              float const*,
...
template void symmetric_quantize<half, half>(int8_t*,
                                             int8_t*,
                                             half*,
                                             half const*,
                                             std::vector<size_t> const&,
...
void symmetric_quantize(int8_t* processed_quantized_weight,
                        ...
                        QuantType quant_type,
                        bool force_interleave) {
  symmetric_quantize(processed_quantized_weight,
                     nullptr,
                     scale_ptr,
                     input_weight_ptr,
                     shape,
...
template void symmetric_quantize<half, half>(
    int8_t*, half*, half const*, std::vector<size_t> const&, QuantType, bool);
