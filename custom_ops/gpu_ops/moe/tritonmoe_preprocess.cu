...
// ============================================================
// tritonmoe_preprocess_with_map: support expert_map filtering
// ============================================================

std::vector<std::vector<int64_t>> tritonmoe_preprocess_with_mapInferShape(
    const std::vector<int64_t>& topk_ids,
    const std::vector<int64_t>& expert_map,
    int64_t num_experts,
    int64_t GEMM_BLOCK_SIZE_M) {
  int topk_ids_numel = topk_ids[0] * topk_ids[1];
  // num_experts here is actually num_local_experts
  int max_num_tokens_padded =
      topk_ids_numel + num_experts * (GEMM_BLOCK_SIZE_M - 1);
...
std::vector<paddle::DataType> tritonmoe_preprocess_with_mapIferDtype(
    const paddle::DataType& topk_ids,
    const paddle::DataType& expert_map,
    int64_t num_experts,
    int64_t GEMM_BLOCK_SIZE_M) {
  return {paddle::DataType::INT32,
          paddle::DataType::INT32,
...
std::vector<paddle::Tensor> tritonmoe_preprocess_with_map_kernel(
    ...
    int64_t GEMM_BLOCK_SIZE_M) {
  int topk_ids_numel = topk_ids.shape()[0] * topk_ids.shape()[1];
  // num_local_experts is the actual number of local experts
  int max_num_tokens_padded =
      topk_ids_numel + num_local_experts * (GEMM_BLOCK_SIZE_M - 1);
  ...
  auto cumsum_buffer = paddle::zeros(
      {num_local_experts + 1}, paddle::DataType::INT32, topk_ids.place());

  auto stream = topk_ids.stream();
  using scalar_t = int64_t;

#define run_align_kernel_with_map(num_experts)                              \
  auto align_kernel = moe_align_block_size_kernel<scalar_t, num_experts>;   \
  align_kernel<<<1, 1024, 0, stream>>>(topk_ids.data<scalar_t>(),           \
                                       expert_ids.data<int32_t>(),          \
                                       num_tokens_post_pad.data<int32_t>(), \
                                       ...
                                       topk_ids_numel,                      \
                                       cumsum_buffer.data<int32_t>(),       \
                                       expert_map.data<int32_t>());

  if (num_local_experts == 8) {
    run_align_kernel_with_map(8);
  } else if (num_local_experts == 64) {
    run_align_kernel_with_map(64);
  } else if (num_local_experts == 128) {
    run_align_kernel_with_map(128);
  } else if (num_local_experts == 256) {
    run_align_kernel_with_map(256);
  } else if (num_local_experts == 2) {
    run_align_kernel_with_map(2);
  } else if (num_local_experts == 32) {
    run_align_kernel_with_map(32);
  } else if (num_local_experts == 160) {
    run_align_kernel_with_map(160);
  } else {
    PD_THROW("Not support num_local_experts: %d", num_local_experts);
  }
...
PD_BUILD_STATIC_OP(tritonmoe_preprocess_with_map)
    .Inputs({"topk_ids", "expert_map"})
    .Attrs({"num_experts: int64_t", "GEMM_BLOCK_SIZE_M: int64_t"})
    .Outputs({"sorted_ids", "expert_ids", "num_tokens_post_pad"})
    .SetKernelFn(PD_KERNEL(tritonmoe_preprocess_with_map_kernel))
    .SetInferShapeFn(PD_INFER_SHAPE(tritonmoe_preprocess_with_mapInferShape))
    .SetInferDtypeFn(PD_INFER_DTYPE(tritonmoe_preprocess_with_mapIferDtype));
